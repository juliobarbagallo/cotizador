...
echo "<h2>Cotizador <b>online</b></h2>";
echo "</div>";
echo "</div>";
echo "</div>";
    ...
    // Escape any html characters
		  $seBorraron = 0;
        $pid = $_POST['delid'];
        $articulos = explode(',', $pid);
        $cantidadArticulos = count($articulos);
        if($cantidadArticulos > 1) {
        	foreach ($articulos as $articulo){
				if(!empty($articulo)) {				
					//echo "-> $articulo <br>";
					mysqli_query($con, "DELETE FROM productos WHERE pid=". $articulo);
					$seBorraron = 1;
				}        
        	}
        }else {
        ...
        if($seBorraron == 1) {
				echo "Los articulos fueron eliminados.";        
        }
echo "</div>";
                	echo "</div>";
